...
<div class="row ">
    <div class="col-md-5 card shade mw-center mh-center">
        ...
        <form action="{{ route('auth.store') }}" method="POST" class="p-2" enctype="multipart/form-data">
			@csrf
			@if ($errors->any())
			...
			<div class="form-group">
				...
				<input type="text" name="name" class="form-control" id="userName"
					placeholder="Basem" value="{{ old('name') }}">
			</div>
			@error('name')
			...
			<div class="form-group">
				...
				<input type="number" name="phone" class="form-control" id="phoneNubmer"
					placeholder="77########" value="{{ old('phone') }}">
			</div>
			@error('phone')
				<small class="text-danger">{{ $message }}</small>
			@enderror                
			...
			<div class="form-group">
				<label for="location">Locatiob</label>
			...
			<div class="c-grey text-center col-3 ">
				<button type="submit" class="btn flat f-first btn-block fnt-xxs ">regester</button>
			</div>
		</form>
		<p>Already have an acount <a href="{{ route('auth.login') }}">login</a></p>
    </div>
